...
public enum FASSETS_lifePolicy implements IBWInterface, IBWSheetInterface{

    DO_YOU_HAVE_ANY_ASSETS(doYouHaveAnyAssets, doYouHaveAnyAssetsYesValue),
    WHAT_IS_TYPE_OF_ASSET(whatIsTypeOfAsset, whatIsTypeOfAssetLifePolicyValue),
    LIFE_POLICY_COMPANY(yourAssetsLifePolicyCompany, null),
    CURRENT_ENCASHMENT_VALUE(yourAssetsCurrentEncashmentValue, null);

    private static final Log log = LogFactory.getLog(FASSETS_lifePolicy.class.getName());
    ...
    public String getSheetName() throws ClassNotFoundException, NoSuchMethodException{
        return ExcelSheetVerificator.YOUR_FINANCIAL_ASSETS_LIFE_POLICY.getSheetName();
    }
    ...
    private static List<String> getExpectedFieldNames() {
        ...
        expectedFieldNames.add(LIFE_POLICY_COMPANY.getFieldName());
        expectedFieldNames.add(CURRENT_ENCASHMENT_VALUE.getFieldName());

        return expectedFieldNames;
    }

    public static List<String> getExpectedFieldNames(String twoBorrowers) {
        List<String> expectedFieldNames = FASSETS_lifePolicy.getExpectedFieldNames();
