...
public enum PERSONAL_DETAILS_subCountry implements IBWInterface {
    ...
    private static final Log log = LogFactory.getLog(PERSONAL_DETAILS_subCountry.class.getName());
    ...
    public static List<String> getExpectedFieldNames(String country){
        List<String> expectedFieldNames = PERSONAL_DETAILS_subCountry.getExpectedFieldNames();

        if(country.equalsIgnoreCase(PERSONAL_DETAILS_subCountry.getTriggerFieldValue())){
            expectedFieldNames.add(COUNTY_STATE.getFieldName());
        }
        return expectedFieldNames;
    }
